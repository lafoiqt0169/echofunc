...
function! EchoFunc()
    let name=strpart(getline('.'), col('.')-3, 2)
    if name !~ '('
        return ''
    endif
...
function! EchoFuncStart()
    ...
    inoremap <silent> <buffer> <C-h><C-j>   <c-r>=EchoFunc()<cr>
...
function! BalloonDeclarationStart()
    let s:balloon_dec_start = 1
    set ballooneval
    set balloonexpr=BalloonDeclaration()
endfunction

function! BalloonDeclarationStop()
    let s:balloon_dec_start = 0
    set balloonexpr=
    set noballooneval
endfunction

function! ToggleBalloonDeclaration()
    if !exists('s:balloon_dec_start')
        call BalloonDeclarationStart()
    elseif (s:balloon_dec_start == 1)
        call BalloonDeclarationStop()
    else
        call BalloonDeclarationStart()
    endif
endfunction
...
command! -nargs=0 BalloonDeclarationToggle  call ToggleBalloonDeclaration()
